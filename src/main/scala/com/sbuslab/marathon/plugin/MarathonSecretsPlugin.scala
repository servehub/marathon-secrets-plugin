...
class MarathonSecretsPlugin
    extends RunSpecTaskProcessor
  ...
  override def initialize(marathonInfo: Map[String, Any],
    ...
    keysPath = (configuration \ "keysPath")
      .asOpt[String]
      .getOrElse("/v1/kv/services/keys")
      .trim
    identitiesPath = (configuration \ "identitiesPath")
      .asOpt[String]
      .getOrElse("/v1/kv/services/sbus/identities")
      .trim
  }
  ...
  def taskInfo(appSpec: ApplicationSpec, builder: TaskInfo.Builder): Unit =
    appSpec.env.get("SERVICE_KEY") collect {
      case serviceId: EnvVarString ⇒
        require(
          appSpec.id.toString.startsWith("/" + serviceId.value),
          throw new Exception("Incorrect $SERVICE_KEY env variable!")
        )
  ...
  private def createIdentityIfMissing(serviceId: EnvVarString) = {
    val resp = (new URL(
      consulAddress + identitiesPath + "/" + serviceId.value + "?raw=true"
    ...
    if (resp.getResponseCode == 404) {
      ...
      consulPut(
        consulAddress + identitiesPath + "/" + serviceId.value,
        Json
          .toJson(Map("groups" → List("service"), "createdAt" → createdAt))
          .toString()
      )
    }
  }
  ...
  private def getOrCreatePrivateKey(serviceId: EnvVarString) = {
    val resp = (new URL(
      consulAddress + keysPath + "/private/" + serviceId.value + "?raw=true"
    ).openConnection()).asInstanceOf[HttpURLConnection]
    ...
    if (resp.getResponseCode == 404) {
      ...
      consulPut(
        consulAddress + keysPath + "/public/" + serviceId.value,
        Json
          .toJson(
            Map(
              "publicKey" → Utils.bytesToHex(
                pair.getPublic.asInstanceOf[EdDSAPublicKey].getAbyte
              ),
              "createdAt" → createdAt
            )
          )
      ...
      val privateSeed =
        Utils.bytesToHex(pair.getPrivate.asInstanceOf[EdDSAPrivateKey].getSeed)

      consulPut(
        consulAddress + keysPath + "/private/" + serviceId.value,
        Json
          .toJson(
            Map(
              "privateKey" → AesPbkdf2.encrypt(encryptKey, privateSeed),
              "createdAt" → createdAt
            )
          )
          .toString()
      ...
      AesPbkdf2.decrypt(
        encryptKey,
        (Json.parse(resp.getInputStream) \ "privateKey").as[String]
      )
    }
  }
  ...
  def taskGroup(podSpec: PodSpec,
                executor: ExecutorInfo.Builder,
  ...
  object AesPbkdf2 {
    ...
    def encrypt(secret: String, text: String): String =
      try {
        val salt: Array[Byte] = new Array[Byte](SALT_SIZE)
        val random: SecureRandom = SecureRandom.getInstanceStrong
        random.nextBytes(salt)

        val key: SecretKey = new SecretKeySpec(
          SecretKeyFactory
            .getInstance("PBKDF2WithHmacSHA256")
            .generateSecret(
              new PBEKeySpec(secret.toCharArray, salt, ITERATIONS, 256)
            )
            .getEncoded,
          "AES"
        )
        ...
        val buf: ByteBuffer =
          ByteBuffer.allocate(1 + SALT_SIZE + IV_SIZE + encrypted.length)
    ...
    def decrypt(secret: String, data: String): String =
      try {
        ...
        val salt: Array[Byte] =
          java.util.Arrays.copyOfRange(encrypted, 1, 1 + SALT_SIZE)
        val iv: Array[Byte] = java.util.Arrays
          .copyOfRange(encrypted, 1 + SALT_SIZE, 1 + SALT_SIZE + IV_SIZE)
        val key: SecretKey = new SecretKeySpec(
          SecretKeyFactory
            .getInstance("PBKDF2WithHmacSHA256")
            .generateSecret(
        ...
        val decrypted: Array[Byte] = cipher.doFinal(
          java.util.Arrays
            .copyOfRange(encrypted, 1 + SALT_SIZE + IV_SIZE, encrypted.length)
        )
